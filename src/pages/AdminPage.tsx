...
const AdminPage = () => {
  ...
  const runImport = async () => {
    ...
    if (!token) {
      toast({ title: 'Devi essere autenticato', variant: 'destructive' });
      return;
    }
    setImporting(true);
    try {
      const r = await fetch('/api/import-offers', {
        method: 'POST',
      ...
      if (!r.ok) {
        try {
          ...
          errMsg = body?.error ?? r.statusText;
        } catch {
          errMsg = r.statusText;
        }
  ...
  const runEvaluateAlerts = async () => {
    ...
    setEvaluating(true);
    try {
      const r = await fetch('/api/evaluate-alerts', {
        method: 'POST',
        headers: { Authorization: 'Bearer ' + token },
      });
      let errMsg: string | undefined;
      ...
      toast({ title: 'Operazione avviata', description: 'Avvisi generati con successo' });
    } catch (err: any) {
