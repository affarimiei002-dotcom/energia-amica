...
/** Fetch open-data.page with retry (3 attempts, 8s timeout each, backoff 1/2/4s). */
async function fetchOpenDataWithRetry(): Promise<string> {
  const delays = [1000, 2000, 4000];
  let lastErr: Error = new Error('Unknown error');
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const resp = await fetchWithTimeout(OPEN_DATA_PAGE, 8000);
...
async function discoverCsvUrls(): Promise<{ electricityUrl: string; gasUrl: string }> {
  const html = await fetchOpenDataWithRetry();
  ...
  const pickLatest = (links: string[]) => links.sort((a, b) => b.localeCompare(a))[0];
  return {
...
/** Retrieve last-known CSV URLs from the most recent successful import_run. */
async function getFallbackUrls(
  supabaseAdmin: ReturnType<typeof createClient>
): Promise<{ electricityUrl: string; gasUrl: string } | null> {
  const { data, error } = await supabaseAdmin
    .from('import_runs')
    .select('errors')
    .in('status', ['success', 'completed'])
    .not('errors->electricity_url', 'is', null)
    .not('errors->gas_url', 'is', null)
    .order('finished_at', { ascending: false })
    .limit(1)
    .single();
  if (error || !data) return null;
  const errors = data.errors as Record<string, string> | null;
  if (!errors || !errors.electricity_url || !errors.gas_url) return null;
  return { electricityUrl: errors.electricity_url, gasUrl: errors.gas_url };
}
...
/** Map tipo_offerta → 'FIXED' | 'VARIABLE'. */
...
serve(async (req) => {
  ...
  try {
    // Try to discover URLs from open-data.page (with retry+timeout)
    try {
      const urls = await discoverCsvUrls();
      electricityUrl = urls.electricityUrl;
      gasUrl = urls.gasUrl;
    } catch (discoverErr: unknown) {
      // Discovery failed — try fallback from last successful import_run
      const fallback = await getFallbackUrls(supabaseAdmin);
      if (!fallback) {
        throw new Error(
          `open-data.page unreachable (${(discoverErr as Error).message}) and no fallback URLs found in import_runs`
        );
      }
      electricityUrl = fallback.electricityUrl;
      gasUrl = fallback.gasUrl;
      usedFallback = true;
      allErrors.push(`open-data.page unreachable: ${(discoverErr as Error).message} — using last-known URLs`);
    }
    ...
    // Persist URLs in errors jsonb so next fallback can use them
    const errorsPayload: Record<string, unknown> = {
      electricity_url: electricityUrl,
      gas_url: gasUrl,
    };
    if (allErrors.length > 0) {
      errorsPayload.errors = allErrors;
    }

    await supabaseAdmin.from('import_runs').update({
      finished_at: new Date().toISOString(),
      status: allErrors.filter(e => !e.startsWith('open-data.page unreachable')).length > 0 && totalCount === 0
        ? 'failure'
        : 'success',
      count: totalCount,
